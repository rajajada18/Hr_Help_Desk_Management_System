	...
	<nav class="navbar navbar-expand-lg bg-dark navbar-dark p-3">
		...
		<div style="border-left: 0.5vh white solid; margin-left: 5%">
			<ul class="navbar-nav mr-auto text-center ml-5 p-2"
				...
				<li class="nav-item pl-5"><a href="/head/view"
					class="nav-link text-uppercase " style="font-size: 2.3vh">List of Tickets <span >
					 <%
					...
					if(count==0)
						...
						out.print("<span class='badge bg-warning text-light rounded-pill ms-4 ml-1 pb-1'> "+count+"</span>");
					else if(count>=10)
						out.print("<span class='badge bg-danger rounded-pill  ms-2 ml-1 pb-1' style='font-size: 1.6vh'> "+count+"</span>");
					%> </span></a></li>
					
				<li class="nav-item  pl-5 ">
					<a class="nav-link text-uppercase " href="/head/user">User Details </a></li>
				<li class="nav-item  pl-5 ">
					<a class="nav-link text-uppercase " href="/head/hr2">HR Details</a></li>
					<li class="nav-item  pl-5 ">
					<a class="nav-link text-uppercase " href="/head/displayReports">User Reports  <span >
					 <%
					/* Object --> Type cast to Integer */
					Integer recount  = (Integer)request.getSession().getAttribute("rCount");
					if(recount<=0)
						out.print("<span class='badge bg-info text-dark rounded-pill ms-2 ml-1 pb-1'> "+recount+"</span>");
					else if(recount>=1 && recount<=9)
						out.print("<span class='badge bg-warning  rounded-pill ms-4 ml-1 pb-1'> "+recount+"</span>");
					else
						out.print("<span class='badge bg-danger rounded-pill ms-2 rounded-sm ml-1 pb-1'> "+recount+"</span>");
					%> </span></a></li>
				<li class="nav-item  pl-5 "><a class="nav-link text-uppercase "
					href="/head/logout">Logout</a></li>
			</ul>
		</div>
	</nav>
	<br>
