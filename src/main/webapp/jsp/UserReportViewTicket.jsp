...
<!-- ##########################Our Code Logic To Display The Tickets ########################## -->	
  ...
  Summary!
    ...
    <a class="btn btn-danger text-dark animated fadeInLeft delay-1s font-weight-bold"href="/report" role="button" id="col" >
  Create New Report
  </a>
</p>
...
<p>	For assistance we might reach you through email.</p>
  ...
  <a class="btn btn-success delay-1s" onclick="gone()">Okay</a>
		...
		<caption>
			The List is about all your tickets raised, which are in active or resolved state. 
			Kindly check on appropriate ticket which gives <em class="btn btn-outline-danger btn-sm disabled" style="pointer-events: none" >`Red Color Report`</em> status for the ticket
			which is in ready state (means the <em>Ticket</em> is eligible to <b class="text-danger">report</b>), else ticket report be in <em class="btn btn-outline-success btn-sm disabled" style="pointer-events:none">`Green Color Report`</em> 
			 (means the <em>Ticket</em> is not eligible to <b class="text-danger">report</b>)
			...
			<tbody class="table-bordered text-center">
					...
					<c:if test="${ticket.status == 'Pending' }">
					<c:if test="${ticket.timediff <= '2'}">
						<p class="text-dark"> <a class="btn btn-success close" href="#" onclick="notallowed()">Report</a> </p>
					</c:if>
				
					
						<c:if test="${ticket.timediff >= '3'}">
						<p class="text-dark"> <a class="btn text-dark btn-danger font-weight-bold" href="../head/report/${ticket.ticketId }/${ticket.timediff}">Report</a> </p>
					</c:if>
	...
	<script>
	var gone = () =>{
		document.getElementById("col").style.transition = "all 1s";
		document.getElementById("col").hidden = "true";
		document.getElementById("collapseExample").style.transition = "all 2s";
